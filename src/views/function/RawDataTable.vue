<template>
	<div class="raw-data-table">
		<div class="raw-content">
			<div class="table-tip">
				<div class="tip">
					<span class="tip-l"></span><span class="tip-t">原数据导出</span><span class="tip-r">产品测试专案</span>
				</div>
				<div class="operation">
			...
			<el-dialog title="BI操作" :visible.sync="visiblebBI">
				<BITABLE ref="bitable" :MetaGuids="case_id" :Questions="subjectIds" :type="1"></BITABLE>
			</el-dialog>
		</div>
	</div>
</template>
	...
	export default {
		...
		methods: {
			...
			DownData() {
				...
				element.setAttribute('href', `/api/BI/exportBI?MetaGuids=${this.case_id}&Questions=${this.subjectIds}`)
				element.setAttribute('download',name)
				element.style.display = 'none'
				document.body.appendChild(element)
				element.click()
				document.body.removeChild(element)
				console.log('下载数据', `/api/BI/exportBI?MetaGuids=${this.case_id}&Questions=${this.subjectIds}`)
